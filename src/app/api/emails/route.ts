...
import { errorJson, okJson, parsePagination, withHeaders } from '@/lib/api-helpers'
import { z } from 'zod'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const session = await prisma.session.findUnique({ where: { fingerprint } })
    if (!session) return errorJson(404, 'Session not found')

    if (id) {
      const email = await prisma.email.findFirst({ where: { id, sessionId: session.id } })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const session = await prisma.session.findUnique({ where: { fingerprint }, include: { emails: true } })
    if (!session) return errorJson(404, 'Session not found')
    ...
    await prisma.session.update({ where: { id: session.id }, data: { emailCount: Math.max(0, session.emailCount - 1) } })
