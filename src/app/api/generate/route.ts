...
import { z } from 'zod'
import { corsHeaders, errorJson, okJson, withHeaders } from '@/lib/api-helpers'
...
export async function POST(request: NextRequest) {
  try {
    const rl = checkRateLimit({ req: request, route: 'POST:/api/generate' })
    ...
    // Add stricter rate limit per fingerprint as well
    const rlfp = checkRateLimit({ req: request, route: 'POST:/api/generate:fp', fingerprint })
...
export async function GET() {
  return okJson({ message: 'Email generation API', endpoints: { POST: '/api/generate' } })
} 